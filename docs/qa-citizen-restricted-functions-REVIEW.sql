...
BEGIN
  -- Registration creates the citizen UUID independently from the random QA email UUID.
  IF p_id IS NULL OR p_email IS NULL
     OR p_email !~ '^fiscalize-qa-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}@example[.]invalid
    RAISE EXCEPTION 'QA_IDENTITY_REJECTED';
  END IF;
  -- Serialize concurrent QA deletion and changes to the target account.
  -- Require the exact ID/email, distinctive synthetic name and a fresh registration.
  PERFORM 1 FROM public.usuarios u
   WHERE u.id = p_id AND u.email = p_email
