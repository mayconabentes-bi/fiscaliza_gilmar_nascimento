      ...
      <section aria-label="Indicadores territoriais">
        <div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-5">
          {cards.map((card, index) => {
            ...
            const wide = index === cards.length - 1;
            return (
              <div key={card.label} className={`rounded-xl border shadow-sm ${wide ? "col-span-2 flex min-h-20 items-center justify-between border-indigo-100 bg-indigo-50/40 px-4 py-3 xl:col-span-1 xl:block xl:border-slate-200 xl:bg-white xl:p-5" : "border-slate-200 bg-white p-4 sm:p-5"}`}>
                <div className={wide ? "xl:hidden" : ""}>
                  {!wide && <Icon className="mb-3 h-5 w-5 text-indigo-600" />}
                  <p className={`${wide ? "font-semibold text-indigo-700" : "text-slate-500"} text-xs sm:text-sm`}>{card.label}</p>
                  <p className={`mt-1 font-bold text-slate-900 ${wide ? "text-2xl" : "text-xl sm:text-2xl"}`}>{card.value == null ? "—" : Number(card.value).toLocaleString("pt-BR")}</p>
                </div>
                {wide && <Icon className="h-6 w-6 text-indigo-600 xl:hidden" />}
                {wide && <div className="hidden xl:block"><Icon className="mb-3 h-5 w-5 text-indigo-600" /><p className="text-sm text-slate-500">{card.label}</p><p className="mt-1 text-2xl font-bold text-slate-900">{card.value == null ? "—" : Number(card.value).toLocaleString("pt-BR")}</p></div>}
              </div>
            );
          })}
        </div>
