...
expect(app.includes('!isInternal && !isCitizen && !onDemandForm && <LGPDConsent />'), 'O aviso público de privacidade deve ficar fora do núcleo privado, da conta autenticada e não competir com o formulário.');
...
expect(app.includes('isSectorStaff ? <>') && app.includes('data-mobile-logout="setor"') && app.includes('aria-label="Sair da área da equipe"'), 'A navegação touch da equipe de setor deve oferecer somente Triagem e Sair.');
...
expect(app.includes('aria-label="Navegação privada"') && app.includes('<DesktopMoreMenu />') && app.includes('label: "Governança"') && app.includes('label: "Equipe"') && app.includes('label: "Auditoria"') && app.includes('label: "Relatórios"'), 'O desktop do admin deve expor apenas módulos privados, com os secundários concentrados em Mais.');
expect(app.includes('whitespace-nowrap rounded-lg px-3 py-2 text-sm font-bold'), 'Itens do menu privado desktop não devem quebrar o nome em duas linhas.');
expect(app.includes('const homeRoute =') && app.includes('isCitizen ? <Navigate to="/meus-registros" replace />') && app.includes('isInternal ? <Navigate to={privateHome} replace />') && app.includes('const privateHome = isAdmin ? "/dashboard" : "/admin/demandas";'), 'Home deve encaminhar admin, equipe de setor e cidadão autenticado para seus contextos próprios.');
expect(app.includes('<Brand privateMode={isInternal} citizenMode={isCitizen} privateHome={privateHome} />') && app.includes('citizenMode ? "/meus-registros"'), 'A marca deve levar usuário interno e cidadão autenticado para seus contextos próprios.');
expect(app.includes('{!isInternal && !isCitizen && <footer'), 'O rodapé público deve ficar fora das experiências autenticadas.');
