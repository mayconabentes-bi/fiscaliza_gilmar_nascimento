...
expect(strategy.includes('Visões da Estratégia') && strategy.includes('>Resumo</button>') && strategy.includes('>Dados</button>') && strategy.includes('>Análises</button>') && strategy.includes('>Plano</button>'), 'Estratégia mobile deve organizar o conteúdo em quatro visões operacionais.');
